...
export async function swapMessagesOrder(conversationId: number, firstMessageId: number, secondMessageId: number) {
    ...
    if (firstMessage && secondMessage) {
        // If both orders is -1, assign their id to the other one's order 
        if (firstMessage.order === -1 && secondMessage.order === -1) {
            [firstMessage.order, secondMessage.order] = [secondMessage.id, firstMessage.id]
        } else if (firstMessage.order === -1) {
            firstMessage.order = secondMessage.order
            secondMessage.order = firstMessage.id
        } else if (secondMessage.order === -1) {
            secondMessage.order = firstMessage.order
            firstMessage.order = secondMessage.id
        } else {
...
export async function updateMessasgeRole(messageId: number, role: Message['role']) {
